...
char* copy_string(char* str, int start, int len);
...
// write(clnt_sock, message, sizeof(message)); //클라이언트에게 데이터를 전송함
	...
	char* send_message;
	int i, j;
	for (i = 0; i < sizeof(message); i += 5) { // 5 bytes 씩 보냄!
		send_message = copy_string(message, i, 5);
		printf("send_message: %s \n", send_message);
		
		write(clnt_sock, send_message, 5);

		free(send_message); // 메모리 할당 해제
		send_message = NULL; // 댕글링 포인터 대비
	}
